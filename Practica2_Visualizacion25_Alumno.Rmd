...
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_line()
grafico7
```

* Modifica el gráfico anterior para que la tendencia de cada fabricante este trazada con un tipo de línea diferente.
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante, linetype=Fabricante))+geom_line()
grafico7
```

* Añade el siguiente título al gráfico *"Ventas mensuales de diferentes compañías tecnológicas"*.
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante, linetype=Fabricante))+geom_line() + labs(title='Ventas mensuales de diferentes compañías tecnológicas')
grafico7
