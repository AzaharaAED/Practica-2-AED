...
grafico1 <-ggplot(ds, mapping= aes(x=MinTemp,y=MaxTemp))
grafico1

ggsave('figs/Ejercicio1.pdf')
```
```{r}
grafico2 <- ggplot(ds,aes(x=MinTemp,y=MaxTemp,color=RainTomorrow))

grafico2+geom_point()
```
```{r}
grafico3 <- grafico2+ ggtitle('Gráfico 3')
...
* Añade títulos explicativos al gráfico anterior (debe aparece un título, subtítulo, leyendas e información de las variables representadas en los ejes). 
* Almacena en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio2.pdf``.

```{r Solucion ejercicio 2}
grafico4 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am)) + geom_point()
grafico4

grafico5 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, shape=RainToday)) + geom_point()
grafico5

grafico6 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, color=RainToday)) + geom_point()+ labs(title='Grafico 6', subtitle='Gráfico de puntos',x='Temp9am',y='Pressure9am')

grafico6
  
ggsave('figs/Ejercicio2.pdf')
```
...
```{r}
iris.grafico2 <- ggplot(data=iris.tidy, mapping=aes(x=Species, y=Value, color=Part)) +geom_point()
iris.grafico2
```
...
```{r}
iris.grafico2 <- ggplot(data=iris.tidy, mapping=aes(x=Species, y=Value, color=Part)) +geom_jitter()
iris.grafico2

ggsave('figs/Ejercicio3.pdf') # se almacena el último gráfico generado
```
...
# El aesthetic group=1 es un agrupamiento "tonto" (da igual si se pone otro valor) para evitar el agrupamiento por defecto que sería la variable x (Dosis), al anular el agrupamiento por defecto todos los valores son del mismo grupo y se unen con una línea. Si x fuese numérica lo haría automáticamente.
...
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()
grafico7
```

* Representa un gráfico como el anterior en el que se muestre la línea que une los puntos que antes has representado.
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()+geom_line()
grafico7
```

* Modifica el gráfico anterior para que la tendencia de cada fabricante este trazada con un tipo de línea diferente.
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()+geom_line(aes(linetype=Fabricante))
grafico7
```

* Añade el siguiente título al gráfico *"Ventas mensuales de diferentes compañías tecnológicas"*.
```{r}
grafico7 <- ggplot(data=DatosEjer4, mapping=aes(x=Fecha, y=Ventas, colour=Fabricante))+geom_point()+geom_line(aes(linetype=Fabricante)) + labs(title='Ventas mensuales de diferentes compañías tecnológicas')
grafico7
ggsave('figs/Ejercicio4.pdf')
```
...
load('data/iceRiver.RData')
library(tseries)

?tseries::ice.river

grafico8 <- ggplot(data=ice.river.df, mapping=aes(x=time))+ geom_line(aes(y=flow.vat),colour='red') +geom_line(aes(y=flow.jok),colour='blue')
#Haces una primera capa con las cosas que son generales y eso arrastrará a las siguientes capas, donde ya especificas lo que quieres en cada linea.
grafico8

```
Como los flujos de cada rio estan representados en diferentes variables, no podemos hacer un grafico lineal simple y establecer que el color de cada linea sea segun el rio. Podríamos reorganizar los datos, creando una variable que se llame flujo donde alacenaríamos los datos de los flujos de ambos rios y luego añadiriamos otra variable llamada rio donde los valores fuesen: vat y jok. Entonces, ya podríamos colorear las lineas según rio.
...
Podemos añadir la curva con la estimación de la distribución de datos con `geom_density`
Pero, ¿Se observa la función de densidad ? Observa la escala `Y`
...
* Descarga de la url  "http://assets.datacamp.com/blog_assets/chol.txt" el dataset de Datacamp que utilizarás para este ejercicio y cárgalo en un datafrae .
...
```{r}
#reorder: reordena el factor(Location) segun el resultado de aplicar FUN a Temp9a
ggplot(data=ds,aes(x=reorder(Location,Temp9am,FUN=mean,decreasing=TRUE), y=Temp9am)) +
  ...
  #annotate permite añadir etiquetas fijas y puntuales en la coordenadas especificadas de un gráfico. En este caso x='Ballarat' es un valor de Location
